...
export default function FractionBridgeRenderer({ level, onComplete }: RendererProps) {
  ...
  const renderFraction = (num: number, den: number, color: string, id: string) => (
    <motion.div 
      layoutId={`fraction-${id}`}
      whileHover={phase === 1 ? { scale: 1.05, y: -5 } : {}}
      className="flex flex-col items-center justify-center text-[#1A2639] rounded-lg w-16 h-20 shadow-[0_4px_0_rgba(0,0,0,0.3)] font-mono font-bold text-xl relative group z-20"
      style={{ backgroundColor: color }}
    >
       <div className="absolute top-1/2 left-2 right-2 h-[2px] bg-[rgba(0,0,0,0.5)] -translate-y-1/2"></div>
       <motion.div key={`num-${num}`} initial={{ opacity: 0, y:-10 }} animate={{ opacity: 1, y:0 }} className="mb-2">{num}</motion.div>
       <motion.div key={`den-${den}`} initial={{ opacity: 0, y:10 }} animate={{ opacity: 1, y:0 }} className="mt-2">{den}</motion.div>
    </motion.div>
  );
    ...
    <div className="flex flex-col h-full bg-[#1A2639] text-[#F5F7FA] p-6 rounded-[16px] overflow-hidden relative">
      ...
      <div className="z-10 flex-1 flex flex-col items-center justify-center max-w-4xl mx-auto w-full">
        <motion.h2 
           ...
           className="text-2xl font-bold mb-12 text-[#E1E5EB] tracking-wide flex items-center gap-3"
        >
          搭建通分桥
        ...
        <div className="w-full flex items-center justify-between px-10 relative">
          ...
          {/* Left Block */}
          {renderFraction(
            phase === 1 ? fracA[0] : (phase >= 2 && result?.isCorrect !== false && parseInt(inputDenom) ? parseInt(inputDenom)/fracA[1]*fracA[0] : fracA[0]), 
            phase === 1 ? fracA[1] : (phase >= 2 && result?.isCorrect !== false && parseInt(inputDenom) ? parseInt(inputDenom) : fracA[1]), 
            "#5BB9B0",
            "A"
          )}
             ...
             <AnimatePresence>
               {phase >= 2 && result?.isCorrect !== false && (
                  <motion.div 
                     ...
                     className="absolute bg-[#F5B041] h-4 rounded shadow-[0_0_20px_#F5B041] w-full border-t border-[#FFF] origin-left"
                  >
               ...
               {phase === 3 && result?.isCorrect === false && (
                  <motion.div 
                     initial={{ scaleX: 0 }}
                     animate={{ scaleX: 0.4, rotate: 15, y: 30 }}
                     transition={{ duration: 0.8, type: "spring" }}
                     className="absolute bg-[#EF7D57] h-4 rounded shadow-[0_0_20px_#EF7D57] w-1/2 border-t border-[#FFF] origin-left opacity-80 left-8"
                  />
               )}
             </AnimatePresence>
          ...
          {/* Right Block */}
          {renderFraction(
            phase === 1 ? fracB[0] : (phase >= 2 && result?.isCorrect !== false && parseInt(inputDenom) ? parseInt(inputDenom)/fracB[1]*fracB[0] : fracB[0]), 
            phase === 1 ? fracB[1] : (phase >= 2 && result?.isCorrect !== false && parseInt(inputDenom) ? parseInt(inputDenom) : fracB[1]), 
            "#EF7D57",
            "B"
          )}
        </div>
        ...
        <div className="mt-16 h-16 flex items-center justify-center">
           {phase === 1 ? (
             <motion.button 
                whileHover={inputDenom ? { scale: 1.05 } : {}}
                whileTap={inputDenom ? { scale: 0.95 } : {}}
                onClick={handleSubmit}
                disabled={!inputDenom}
                className={`flex items-center gap-2 px-8 py-3 rounded-full font-bold text-lg transition-colors 
                  ${inputDenom ? 'bg-[#4C8DA8] text-white shadow-[0_4px_0_#2A657D]' : 'bg-[#2D3E5A] text-[#5A6D8C] opacity-50 cursor-not-allowed border-transparent shadow-none'}
                `}
             >
             ...
             <motion.div 
                ...
                className={`flex items-center gap-3 px-6 py-3 rounded-xl border ${result?.isCorrect ? 'bg-[rgba(91,185,176,0.1)] border-[#5BB9B0] text-[#5BB9B0]' : 'bg-[rgba(239,125,87,0.1)] border-[#EF7D57] text-[#EF7D57]'}`}
             >
             ...
             <motion.div 
               ...
               className="text-[#8A9EB8] font-mono"
             >
               正在推演架构变形...
             </motion.div>
           )}
