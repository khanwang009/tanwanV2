    ...
    <div className="flex flex-col h-full bg-[#1A2639] text-[#F5F7FA] p-6 rounded-[16px] shadow-[inset_0_0_40px_rgba(0,0,0,0.5)] overflow-hidden">
      <div className="flex-1 flex flex-col md:flex-row gap-8 items-center justify-center">
        ...
        <div className="flex-1 flex items-center justify-center relative w-full aspect-square md:aspect-auto">
           ...
           <motion.div 
             animate={submitted && resultProps?.isCorrect ? { rotateY: 360, rotateX: 360 } : {}}
             transition={{ duration: 2, ease: "easeInOut" }}
             className="relative w-64 h-64 transform rotate-x-12 rotate-y-[-10deg] perspective-[1500px] transform-style-3d group"
           >
             <AnimatePresence>
               {faces.map(face => {
                 ...
                 return (
                   <motion.div 
                     ...
                     className={`absolute inset-0 border-2 transition-all duration-300 backdrop-blur-sm flex items-center justify-center
                       ...
                       ${submitted && isSelected && !targetFaces.includes(face.id) ? 'bg-[rgba(239,125,87,0.4)] border-[#EF7D57]' : ''}
                     `}
        ...
        <motion.div 
           ...
           className="w-full md:w-80 bg-[#24354F] p-6 rounded-2xl border-t border-l border-[#3D5275] shadow-2xl flex flex-col gap-6"
        >
          ...
          <div className="grid grid-cols-2 gap-3">
             {faces.map((face, idx) => (
               <motion.button
                 ...
                 disabled={submitted}
                 className={`py-3 px-4 rounded-lg font-bold text-sm transition-colors border-b-2
          ...
          <motion.button 
            whileHover={!submitted && selectedFaces.length > 0 ? { scale: 1.05 } : {}}
            whileTap={!submitted && selectedFaces.length > 0 ? { scale: 0.95 } : {}}
            onClick={handleSubmit} 
            disabled={submitted || selectedFaces.length === 0}
            className={`mt-4 py-4 rounded-xl font-bold text-lg shadow-lg transition-colors
              ${submitted 
          ...
          >
            {submitted 
              ? (resultProps?.isCorrect ? <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} className="flex items-center justify-center gap-2"><Check className="w-5 h-5"/> 成功</motion.span> : <motion.span initial={{ scale: 0 }} animate={{ scale: 1 }} className="flex items-center justify-center gap-2"><AlertCircle className="w-5 h-5"/> 结构错误</motion.span>)
              : '提交材料'
            }
          </motion.button>
        </motion.div>
